...
/***************************/
//notice that cpoy is allocating space dinmicly
typedef void* (*copy_func_g)(const void * src);
typedef void (*free_data_g)(void * src);
typedef bool (*comp_func_g)(const void * a, const void * b);
typedef void (*print_func_g)(void const *to_print);
typedef bool (*is_last_g)(const void * b);
...
typedef struct MarkovNode{
 ...
 //1 if end of sentence else 0
 int end;
 // number of diffrent words in frequency list
 int diff_words;
...
typedef struct MarkovChain {
    ...
    copy_func_g copy_func;

    comp_func_g comp_func;

    free_data_g free_data;

    print_func_g print_func;

    is_last_g is_last_func;
} MarkovChain;
