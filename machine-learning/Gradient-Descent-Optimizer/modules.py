...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        if self.include_intercept_:
            reg_val = np.sum(regularization[1:])
        else:
            reg_val = np.sum(regularization)
        return fid + self.lam_ * reg_val
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        fid_gradient = self.fidelity_module_.compute_jacobian(**kwargs)
        regularization_gradient = self.regularization_module_.compute_jacobian(**kwargs)

        if self.include_intercept_:
            # Add zero for intercept so shapes match
            reg_grad = np.concatenate([[0.], regularization_gradient])
        else:
            reg_grad = regularization_gradient

        return fid_gradient + self.lam_ * reg_grad
    @property
    def weights(self):
        """
