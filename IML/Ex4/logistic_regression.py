...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        d = X.shape[1]
        init_w = np.random.normal(loc=0, scale=1/np.sqrt(d), size=d)
        logistic_module = LogisticModule(weights=init_w)
        reg =None


        if self.penalty_ != "none":
            if self.penalty_ == "l1":
                reg = L1(weights=init_w)
            elif self.penalty_ == "l2":
                reg = L2(weights=init_w)
            logistic_module = RegularizedModule(logistic_module,
                                           reg,
                                           lam=self.lam_,
                                           weights=init_w,
                                           include_intercept=self.include_intercept_)
        self.coefs_ = self.solver_.fit(logistic_module, X, y)
